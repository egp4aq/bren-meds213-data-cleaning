...
when we initially explore the data, we see that the Snow_cover column that we are interested in is of type character. 
...
snowsurvey_fixed <- snowsurvey_csv %>% 
  mutate(Snow_cover = ifelse(Snow_cover=='.', NA, c(Snow_cover,Water_cover, Land_cover, Total_cover)))
```
...
# when looking at the metadata, we see that we want all three of our covers to add up to our total_cover (duh!) so for our one row that has the value <1 for Snow_cover, we see that the other two columns add up to the total cover. So, we can just make the Snow_cover equal to 0. 
```
